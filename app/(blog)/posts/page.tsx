...
function isTechnologyNews(post: Awaited<ReturnType<typeof getPostListing>>[number]) {
  const normalizedTags = post.tags.map((tag) => tag.toLocaleLowerCase("vi-VN"));
  return (
    post.category === "Khám phá" ||
    normalizedTags.some((tag) => ["news", "tin-tuc", "tin tức", "cong-nghe", "công nghệ"].includes(tag))
  );
}
...
export async function generateMetadata({ searchParams }: PostsPageProps): Promise<Metadata> {
  ...
  const hasFilters = Boolean(
    getSingleSearchParam(params.category) || getSingleSearchParam(params.feed) || getSingleSearchParam(params.q),
  );
...
export default async function PostsPage({ searchParams }: PostsPageProps) {
  ...
  const activeFeed = requestedFeed === "devinsight" || requestedFeed === "news" ? requestedFeed : undefined;
  const allPosts = await getPostListing();
  const normalizedQuery = query.toLocaleLowerCase("vi-VN");
  const posts = allPosts.filter((post) => {
    const matchesCategory = !activeCategory || post.category === activeCategory;
    const matchesFeed = !activeFeed || (activeFeed === "news" ? isTechnologyNews(post) : !isTechnologyNews(post));
    const searchableContent = [post.title, post.excerpt, post.category, post.author.name, ...post.tags]
      .join(" ")
      ...
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
        <header className="border-b border-[#CBD5E1] pb-5 sm:pb-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[#64748B]">
            Hướng dẫn để học và làm tốt hơn, cùng các tin tức công nghệ đáng theo dõi.
          </p>

          <form action="/posts" className="mt-4 flex max-w-2xl gap-2" role="search">
            {activeCategory ? <input type="hidden" name="category" value={activeCategory} /> : null}
            {activeFeed ? <input type="hidden" name="feed" value={activeFeed} /> : null}
            ...
            <button type="submit" className="h-10 rounded-lg bg-[#1E293B] px-4 text-xs font-bold text-white hover:bg-[#334155] focus:outline-none focus:ring-2 focus:ring-[#7C3AED] focus:ring-offset-2">
              Tìm kiếm
            </button>
          </form>
        </header>
        ...
        <nav className="mt-4 flex flex-wrap gap-2" aria-label="Lọc theo dòng nội dung">
          {[
            { label: "Tất cả nội dung", feed: undefined, count: allPosts.length },
            { label: "Bài viết DevInsight", feed: "devinsight" as const, count: allPosts.filter((post) => !isTechnologyNews(post)).length },
            { label: "Tin tức công nghệ", feed: "news" as const, count: allPosts.filter(isTechnologyNews).length },
          ].map((item) => {
        ...
        <nav className="mt-3 flex flex-wrap gap-x-3 gap-y-1.5 text-xs font-semibold" aria-label="Lọc bài viết theo chuyên mục">
          <Link href={createPostsHref({ feed: activeFeed, query })} className={!activeCategory ? "text-[#6D28D9]" : "text-[#64748B] hover:text-[#6D28D9]"}>
            Tất cả chuyên mục
          </Link>
          {postCategories.map((category) => (
            <Link
              ...
              className={activeCategory === category ? "text-[#6D28D9]" : "text-[#64748B] hover:text-[#6D28D9]"}
            >
              {category}
            </Link>
          ))}
        ...
        <div className="mt-5 flex items-center justify-between gap-4 text-xs font-semibold text-[#64748B]">
          ...
          {(activeCategory || activeFeed || query) && (
            <Link href="/posts" className="inline-flex items-center gap-1 text-[#6D28D9] hover:underline">
          ...
          <section className="mt-4 grid overflow-hidden rounded-xl border border-[#CBD5E1] bg-white lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,0.8fr)]">
            ...
            <div className="flex flex-col p-4 sm:p-5">
              ...
              <div className="mt-auto flex items-center justify-between gap-3 pt-4 text-xs font-semibold text-[#64748B]">
                <span className="inline-flex items-center gap-1"><Clock className="h-3.5 w-3.5 text-[#7C3AED]" />{featuredPost.readingTime}</span>
                <Link href={`/posts/${featuredPost.slug}`} className="inline-flex items-center gap-1 font-bold text-[#6D28D9] hover:underline">
        ...
        {latestPosts.length > 0 ? (
          <section className="mt-7">
            <div className="flex items-center justify-between gap-4">
              <h2 className="text-base font-extrabold text-[#1E293B]">Mới cập nhật</h2>
              <span className="text-xs font-semibold text-[#64748B]">Theo thời gian xuất bản</span>
            </div>
            <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {latestPosts.map((post) => <PostListItem key={post.slug} post={post} />)}
            </div>
          </section>
        ) : null}
        ...
        {activeFeed === "devinsight" ? (
          <section className="mt-8">
            <PostFeed
              ...
              posts={devinsightPosts.filter((post) => post.slug !== featuredPost?.slug)}
          ...
          <section className="mt-8">
            <PostFeed
              ...
              posts={technologyNews.filter((post) => post.slug !== featuredPost?.slug)}
        ...
        {!posts.length ? (
          <section className="mt-5 rounded-xl border border-dashed border-[#94A3B8] bg-white px-6 py-10 text-center">
            <h2 className="text-base font-extrabold text-[#1E293B]">Chưa tìm thấy nội dung phù hợp</h2>
            <p className="mt-1.5 text-xs text-[#64748B]">Thử một từ khóa khác hoặc bỏ bớt bộ lọc.</p>
            <Link href="/posts" className="mt-3 inline-flex text-xs font-bold text-[#6D28D9] hover:underline">Xem toàn bộ nội dung</Link>
          </section>
        ) : null}
      </div>
    </main>
  ...
  return (
    <section>
      ...
      {posts.length ? (
        ...
        <p className="mt-3 rounded-lg border border-dashed border-[#CBD5E1] bg-white px-3 py-4 text-xs leading-5 text-[#64748B]">{emptyMessage}</p>
      )}
    </section>
  );
}
