...
export default async function AdminDashboardPage() {
  ...
  const cards = [
    {
      label: "Tổng lượt đọc",
      value: analytics.totalReaders,
      icon: Eye,
      color: "bg-[#8B5CF6]",
    },
    {
      label: "Độc giả hôm nay",
      value: analytics.readersToday,
      icon: Activity,
      color: "bg-[#F472B6]",
    },
    {
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {cards.map(({ label, value, icon: Icon, color }) => (
          ...
          >
            <span
              className={`grid h-10 w-10 place-items-center rounded-xl border-2 border-[#1E293B] text-white ${color}`}
            >
        ...
        <article className="rounded-2xl border-2 border-[#1E293B] bg-white p-6 shadow-pop-sm lg:col-span-3">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-xl font-extrabold">
                Độc giả 7 ngày gần nhất
              </h2>
              <p className="mt-1 text-sm text-[#64748B]">
                Lượt đọc duy nhất theo từng ngày
              </p>
            </div>
          ...
          <p className="mt-5 text-sm text-[#64748B]">
            ...
            <strong className="text-[#1E293B]">
              {numberFormat.format(analytics.readersThirtyDays)}
            </strong>{" "}
            độc giả duy nhất.
          </p>
        </article>
                ...
                <li key={post.slug} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    <Link
                      ...
                      className="line-clamp-2 font-bold hover:text-[#8B5CF6]"
                    >
                      {post.title}
                    </Link>
                    <p className="mt-1 text-xs text-[#64748B]">
                      {numberFormat.format(post.view_count)} lượt đọc
                    </p>
                  </div>
                </li>
              ))
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <article className="rounded-2xl border-2 border-[#1E293B] bg-white p-6 shadow-pop-sm">
          ...
          <div className="mt-5 space-y-3">
            {platform.latestUsers.map((user) => (
              <div
                key={user.id}
                className="flex items-center justify-between gap-3 rounded-xl bg-[#F1F5F9] p-3"
              >
                <div className="min-w-0">
                ...
                <span
                  className={`rounded-full px-2 py-1 text-[10px] font-bold ${user.role === "ADMIN" ? "bg-[#FBBF24] text-[#1E293B]" : "bg-white text-[#64748B]"}`}
                >
                  {user.role}
                </span>
              </div>
        ...
        <article className="rounded-2xl border-2 border-[#1E293B] bg-[#1E293B] p-6 text-white shadow-pop-sm">
          ...
          <dl className="mt-5 grid grid-cols-2 gap-4 text-sm">
            <div>
              <dt className="text-white/60">Quản trị viên</dt>
              <dd className="mt-1 text-2xl font-extrabold">
                {platform.adminUsers}
              </dd>
            </div>
            <div>
              <dt className="text-white/60">Email đã xác thực</dt>
              <dd className="mt-1 text-2xl font-extrabold">
                {platform.verifiedUsers}
              </dd>
            </div>
            <div>
              <dt className="text-white/60">Thành viên mới 30 ngày</dt>
              <dd className="mt-1 text-2xl font-extrabold">
                {platform.newUsersLastThirtyDays}
              </dd>
            </div>
            <div>
              <dt className="text-white/60">Bài lưu trữ</dt>
              <dd className="mt-1 text-2xl font-extrabold">
                {platform.archivedPosts}
              </dd>
            </div>
          </dl>
